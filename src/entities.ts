...
type PlayerGameObj = GameObj<SpriteComp & AreaComp & BodyComp & PosComp & ScaleComp & DoubleJumpComp & HealthComp & OpacityComp & {
    speed: number;
    direction: string;
    isInhaling: boolean;
    isExhaling: boolean;
...
export function setControls(k: KaboomCtx, player: PlayerGameObj) {
            ...
            case "z":
                if (player.isFull) {
                    ...
                    const shootingStar = k.add([
                        ...
                        k.scale(scale),
                        k.move(player.direction === "left" ? k.LEFT : k.RIGHT, 800),
                        "shootingStar",
                    ]);
    ...
    k.onKeyRelease((key) => {
        if (key === "z") {
            ...
            if (!player.isFull && !player.isExhaling) {
                player.play("kirbIdle");
            }
        }
    });
...
export function makeBirdEnemy(k: KaboomCtx, posX: number, posY: number, speed: number) {
    const bird = k.add([
        ...
        // Prevents gravity from affecting movement
